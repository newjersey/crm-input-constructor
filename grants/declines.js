...
const optionDefinitions = [
  ...
  { name: 'out', alias: 'o', type: String, description: 'Path to output file; suggest naming like hard_declines_0-500.json' },
  ...
  const usage = commandLineUsage([
    {
      ...
      content: 'This takes grant data from a CSV input file and outputs a corresponding JSON array of objects for feeding to our Power Automate flow that sends decline emails.'
    },
    {
      header: 'Options',
...
function reasons(application) {
  const FINDING_DEFINITIONS = [
    // IMMEDIATE DECLINE
    { trigger: 'Ineligible Entity Type', language: `Entity is an ineligible entity type based on the Guidelines` },
    { trigger: 'Ineligible Founding Year', language: `Entity was not in existence in the 4th quarter of 2019` },
    { trigger: 'Home Based w/o Commercial Location', language: `Entity is a home-based entity without a physical commercial location` },
    { trigger: 'Ineligible NAICS and Sector', language: `Entity is not in an eligible industry based on the Guidelines` },
    { trigger: 'BusinessDetails_GamblingActivities', language: `Entity is considered an ineligible entity because it hosts gambling or gaming activities` },
    { trigger: 'BusinessDetails_AdultActivities', language: `Entity is considered an ineligible entity because it conducts or purveys “adult” activities, services, products or materials` },
    { trigger: 'BusinessDetails_SalessActivities', language: `Entity is considered an ineligible entity because it conducts auctions, bankruptcy sales, fire sales, “lost-our-lease,” “going-out-of-business,” or similar sales` },
    { trigger: 'BusinessDetails_TransientMerchant', language: `Entity is considered an ineligible entity because it is a transient merchant ("peddler," "popup store," or "itinerant vendor")` },
    { trigger: 'BusinessDetails_OutdoorStorageCompany', language: `Entity is considered an ineligible entity becauseis an outdoor storage company` },
    { trigger: 'BusinessDetails_NuisanceActivities', language: `Entity is considered an ineligible entity because it conducts activities that may constitute a nuisance` },
    { trigger: 'BusinessDetails_IllegalActivities', language: `Entity is considered an ineligible entity because it conducts business for an illegal purpose` },
    { trigger: 'Too Few FTE', language: `Entity did not have at least 1 full-time equivalent employee in the 4th Quarter of 2019, based on the calculation specified in the Guidelines` },
    { trigger: 'Too Many FTE', language: `Entity had ${application['Rounded FTE']} full-time equivalent employees in the 4th Quarter of 2019, based on the calculation specified in the Guidelines, which exceeds the maximum of 10 allowed for this program` },
  ...
  return FINDING_DEFINITIONS
    .filter(obj => bool(application[obj.trigger]))
    .map(obj => obj.language);
}

function businessNameWithDBA(application) {
  return application['Taxation Name'].trim() + (
...
function generateObject(application) {
  return {
    "application_id": application.__PrefixedAppID__.trim(),
    ...
    "city_state_zip": `${application.normalized_city.trim()}, NJ ${application.ContactInformation_BusinessAddress_PostalCode.padStart(5, '0').trim()}`,
    "reasons": reasons(application),
  };
}
