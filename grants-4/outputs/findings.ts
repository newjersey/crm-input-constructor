...
import { addABCData } from '../inputs/ABC';
import {
...
import { YesNo } from '../../grants-2/inputs/xlsx';
...
const FINDING_DEFINITIONS: FindingDef[] = [
  // yesNo(app.sams.possibleMatches.length > 0), // TODO: if yes, add details to findings

    {
        ...
        trigger: app => (app.Business_EntityType_Value === EntityType.Other || app.Business_EntityType_Value === EntityType.Government_Body),
        messageGenerator: app =>
            `Ineligible Business Entity Type: "Other (estate, municipality, Government Body, etc.)"`,
        publicMessageGenerator: app =>
            `Entity is considered an ineligible entity type (e.g. estate, municipality, Government Body, etc.)`,
    ...
    },
    {
        ...
        trigger: app =>
            ...
            isAfter(dateFromExcel(app.Business_DateEstablished), new Date(2020, 1, 15)),
        messageGenerator: app =>
    ...
    },
    {
        ...
        trigger: app => (getQuarterlyWageData(app).fteCount || 0) > 50,
        messageGenerator: app =>
            `Too Many FTE Equivalents: ${getQuarterlyWageData(app).fteCount} but should be at most 50`,
        publicMessageGenerator: app =>
            `Entity had over 50 full-time equivalent employees on their most recently filed NJ WR-30 filed with the NJ Department of Labor and Workforce Development, based on the calculation specified in the Guidelines`,
        severity: Decision.Decline,
        slug: 'HighFTE',
    },
    {
    ...
    // },
    {
        ...
        messageGenerator: app =>
            `EIN was found in more than one application (${app.DuplicateEIN.DuplicateApplicationIDs})`,
        publicMessageGenerator: app => `Tax Identification Number (TIN) reported on application was found in a previous application`,
        severity: Decision.Decline,
        slug: 'DupEIN',
    },
  ...
  // New Decline  add on 20201103
    {
        ...
        ,
        messageGenerator: app =>
            `Business is not operational or no plan to Reopen`,
...

    {
        ...
        ,
        messageGenerator: app =>
            `No Need after other assistance`,
...

    {
        ...
        ,
        messageGenerator: app =>
            `Ineligible Use of Grants proceeds`,
    ...
    //Child Care
    {
        ...
        && ((app.njeda_revisedischildcarecenterproperlylicensed !== undefined && app.njeda_revisedischildcarecenterproperlylicensed == 'No')
           || (app.njeda_revisedischildcarecenterproperlylicensed === undefined && app.njeda_ischildcarecenterproperlylicensedorexempt == 'No')), // No
        messageGenerator: app => `Child Care and NOT Licensed as per applicant and NOT in the DCF-DHS Licensed List`,
        publicMessageGenerator: app =>
            `Child Care and NOT Licensed as per applicant and NOT in the DCF-DHS Licensed List`,
        severity: Decision.Decline,
  ...
  // New for Grant-4
    ...
    //Child Care
    {
        ...
        || (app.njeda_revisedischildcarecenterproperlylicensed === undefined && app.njeda_ischildcarecenterproperlylicensedorexempt == 'Yes')), // Yes
    ...
    },
    {
        ...
        || (app.njeda_revisedchildcarecenterexemptreason === undefined && app.njeda_childcarecenterexemptreason == 'None of the Above.')), // None of Above
    ...
    //DOL WHD NO EIN NO GO 
    {
        ...
        messageGenerator: app =>
            `Applicant may be on the DOL WHD NO GO List based on 85% tolerance.`,
        severity: Decision.Review,
        slug: 'DOL_NoEIN_NOGO',
    },
    {
        ...
        messageGenerator: app =>
            `Applicant stated their Business is not properly registered with the state`,
        severity: Decision.Review,
    ...
    {  
        ...
        messageGenerator: app =>
            `Applicant not found at Taxation`,
        severity: Decision.Review,
    ...
    //Organization Name Compare with Taxation REG name with 35% tolerance --> not matches, push to Manual Review
    {
        ...
        messageGenerator: app =>
            `Registered Business Name "${app.taxation["TAXREG Name"].trim()}" does not match with Applicant provided name. And Match percentage is "${Math.round(stringSimilarity.compareTwoStrings(app.ContactInformation_BusinessName.trim().toUpperCase(), app.taxation["TAXREG Name"].trim().toUpperCase()) * 100) / 100}%" `,
        severity: Decision.Review,
  ...
  // New on Grant-3
     ...
     //ABC
    {
        ...
        messageGenerator: app =>
            `Applicant may be on the ABC violations list: ${app.ABC.possibleMatches
    ...
    //Revised Unmet Need < Original Unmet Need & Revised unmet Need < Award Size caluclated
    {
        ...
        trigger: app => (!!app.njeda_additionalbusinessfundingneeded && !!app.njeda_revisedbusinessfundingneeded && app.njeda_revisedbusinessfundingneeded < app.njeda_additionalbusinessfundingneeded && app.njeda_revisedbusinessfundingneeded < AwardBasis(app)),
    ...
    //No WR30 Highest FTE Jobs count Data and FEIN not on DOL registered list and Business Type in Restaurant, Small & Child Care
    {
        ...
        trigger: app => ((getQuarterlyWageHighestFTE(app).fteCount == 0 || (getQuarterlyWageHighestFTE(app).fteCount == null)) && !app.dol.isActiveEmployer
    ...
    // },
    {
        ...
        messageGenerator: app =>
            `Business address was found in more than one application (${app.DuplicateAddress.DuplicateApplicationIDs})`,
        severity: Decision.Review,
        slug: 'DupAddress',
    },
    ...
    //DOB No EIN Review 
    {
        ...
        messageGenerator: app =>
            `Applicant may be on the DOB No EIN List based on 85% tolerance.`,
        severity: Decision.Review,
    ...
    //DOB Sister Agencies Check
    {
        ...
        messageGenerator: app =>
            `On DOB Sister Agencies List (DHS or DCA or Arts) and Amount doesnt match.`,
        severity: Decision.Review,
    ...
    
    {
        ...
        messageGenerator: app =>
            `Applicant reported SBA PPP funding approved or in progress but did not indicate the amount`,
        severity: Decision.Review,
    ...
    },
    {
        ...
        trigger: app => app.DOBValidation.DOBRecords.length > 0
            && app.DOBValidation.DOBFindings.includes(DOBPrograms.PPP)
            && ((app.DOBAffidavit_SBAPPP !== "Yes" && app.njeda_sbapaycheckprotectionprogramphase2 !== "Yes")
            || (app.DOBValidation.DOBAmounts.search(DOBPrograms.PPP) > 0 ? (Math.round(parseInt(app.DOBValidation.DOBAmounts.substr(app.DOBValidation.DOBAmounts.search(DOBPrograms.PPP) + DOBPrograms.PPP.length
        ...
        messageGenerator: app =>
            `Applicant either not listed on Application or less amount mentioned, but in DOB List (SBA PPP).`,
        severity: Decision.Review,
    ...
    },
    {
        ...
        messageGenerator: app =>
            `Applicant reported SBA PPP2 funding approved or in progress but did not indicate the amount`,
        severity: Decision.Review,
        slug: 'MissingDOBAmount',
    },
    {
    ...
    },
    {
        ...
        messageGenerator: app =>
            `Applicant reported EIDL funding approved or in progress but did not indicate the amount`,
        severity: Decision.Review,
        slug: 'MissingDOBAmount',
    },
    {
        ...
        messageGenerator: app =>
            `Applicant reported Loan Assistance funding approved or in progress but did not indicate the amount`,
        severity: Decision.Review,
        slug: 'MissingDOBAmount',
    },
    {
        ...
        messageGenerator: app =>
            `Applicant reported Grant Assistance Phase 1 funding approved or in progress but did not indicate the amount`,
        severity: Decision.Review,
        slug: 'MissingDOBAmount',
    },
    {
        ...
        trigger: app => app.DOBValidation.DOBRecords.length > 0
            && app.DOBValidation.DOBFindings.includes(DOBPrograms.NJEDAPhase1)
            && (app.DOBAffidavit_NJEDAGrant !== "Yes"
            || (app.DOBValidation.DOBAmounts.search(DOBPrograms.NJEDAPhase1) > 0 ? (Math.round(parseInt(app.DOBValidation.DOBAmounts.substr(app.DOBValidation.DOBAmounts.search(DOBPrograms.NJEDAPhase1) + DOBPrograms.NJEDAPhase1.length
        ...
        messageGenerator: app =>
            `Applicant either not listed on Application or less amount mentioned, but in DOB List (Grant Phase 1).`,
        severity: Decision.Review,
    ...
    },
    {
        ...
        messageGenerator: app =>
            `Applicant reported Grant Assistance Phase 2 funding approved or in progress but did not indicate the amount`,
        severity: Decision.Review,
        slug: 'MissingDOBAmount',
    },
    {
        ...
        trigger: app => app.DOBValidation.DOBRecords.length > 0
            && app.DOBValidation.DOBFindings.includes(DOBPrograms.NJEDAPhase2)
            && (app.njeda_njedasbemergencygrantassistancephase2 !== "Yes"
            || (app.DOBValidation.DOBAmounts.search(DOBPrograms.NJEDAPhase2) > 0 ? (Math.round(parseInt(app.DOBValidation.DOBAmounts.substr(app.DOBValidation.DOBAmounts.search(DOBPrograms.NJEDAPhase2) + DOBPrograms.NJEDAPhase2.length
        ...
        messageGenerator: app =>
            `Applicant either not listed on Application or less amount mentioned, but in DOB List (Grant Phase 2).`,
        severity: Decision.Review,
    ...
    },
    {
        ...
        messageGenerator: app =>
            `Applicant reported Grant Assistance Phase 3 funding approved or in progress but did not indicate the amount`,
        severity: Decision.Review,
        slug: 'MissingDOBAmount',
    },
    {
        ...
        trigger: app => app.DOBValidation.DOBRecords.length > 0
            && app.DOBValidation.DOBFindings.includes(DOBPrograms.NJEDAPhase3)
            && (app.njeda_njedasbegrantassistancephase3 !== "Yes"
            || (app.DOBValidation.DOBAmounts.search(DOBPrograms.NJEDAPhase3) > 0 ? (Math.round(parseInt(app.DOBValidation.DOBAmounts.substr(app.DOBValidation.DOBAmounts.search(DOBPrograms.NJEDAPhase3) + DOBPrograms.NJEDAPhase3.length
        ...
        messageGenerator: app =>
            `Applicant either not listed on Application or less amount mentioned, but in DOB List (Grant Phase 3).`,
        severity: Decision.Review,
    ...
    },
    {
        ...
        messageGenerator: app =>
            `Applicant reported CARES Act Funding From Local County funding approved or in progress but did not indicate the amount`,
        severity: Decision.Review,
        slug: 'MissingDOBAmount',
    },
    {
        ...
        messageGenerator: app =>
            `Applicant reported Housing and Mortgage Finance Covid-19 Landlord Grant funding approved or in progress but did not indicate the amount`,
        severity: Decision.Review,
        slug: 'MissingDOBAmount',
    },
    {
        ...
        trigger: app => app.DOBValidation.DOBRecords.length > 0
            && app.DOBValidation.DOBFindings.includes(DOBPrograms.HMFAGrant)
            && (app.njeda_njhandmortgagefinancecovid19landlordgrant !== "Yes"
            || (app.DOBValidation.DOBAmounts.search(DOBPrograms.HMFAGrant) > 0 ? (Math.round(parseInt(app.DOBValidation.DOBAmounts.substr(app.DOBValidation.DOBAmounts.search(DOBPrograms.HMFAGrant) + DOBPrograms.HMFAGrant.length
        ...
        messageGenerator: app =>
            `Applicant either not listed on Application or less amount mentioned, but in DOB List (HMFA Grant).`,
        severity: Decision.Review,
    ...
    },
    {
        ...
        messageGenerator: app =>
            `Applicant reported Redevelopment Authority Small Business Lease Emergency funding approved or in progress but did not indicate the amount`,
        severity: Decision.Review,
        slug: 'MissingDOBAmount',
    },
    {
        ...
        trigger: app => app.DOBValidation.DOBRecords.length > 0
            && (app.DOBValidation.DOBFindings.includes(DOBPrograms.NJRAPhase1) || app.DOBValidation.DOBFindings.includes(DOBPrograms.NJRAPhase2))
            && ((app.njeda_njredevauthsbleaseemergassistgrantprogram !== "Yes" && app.njeda_njrasbleaseeagprogphase2 !== "Yes")
                || (app.DOBValidation.DOBAmounts.search(DOBPrograms.NJRAPhase1) > 0 ? (Math.round(parseInt(app.DOBValidation.DOBAmounts.substr(app.DOBValidation.DOBAmounts.search(DOBPrograms.NJRAPhase1) + DOBPrograms.NJRAPhase1.length
                ...
                || (app.DOBValidation.DOBAmounts.search(DOBPrograms.NJRAPhase2) > 0 ? (Math.round(parseInt(app.DOBValidation.DOBAmounts.substr(app.DOBValidation.DOBAmounts.search(DOBPrograms.NJRAPhase2) + DOBPrograms.NJRAPhase2.length
        ...
        messageGenerator: app =>
            `Applicant either not listed on Application or less amount mentioned, but in DOB List (NJRA Phase 1 or Phase 2).`,
        severity: Decision.Review,
    ...
    },
    {
        ...
        messageGenerator: app =>
            `Applicant reported Redevelopment Phase 2 Authority Small Business Lease Emergency funding approved or in progress but did not indicate the amount`,
        severity: Decision.Review,
        slug: 'MissingDOBAmount',
    },
    {
        ...
        messageGenerator: app =>
            `Applicant reported Insurance Proceeds funding approved or in progress but did not indicate the amount`,
        severity: Decision.Review,
        slug: 'MissingDOBAmount',
    },
    {
...

    {
        ...
        messageGenerator: app =>
            `Applicant reported PPE funding approved or in progress but did not indicate the amount`,
        severity: Decision.Review,
        slug: 'MissingDOBAmount',
    },
    {
        ...
        trigger: app => app.DOBValidation.DOBRecords.length > 0
            && app.DOBValidation.DOBFindings.includes(DOBPrograms.NJEDAPPE)
            && ((app.njeda_njpersonalppeaccessprogram !== "Yes" && app.njeda_njedasmbppeaccessprogphase2 !== "Yes")
            || (app.DOBValidation.DOBAmounts.search(DOBPrograms.NJEDAPPE) > 0 ? (Math.round(parseInt(app.DOBValidation.DOBAmounts.substr(app.DOBValidation.DOBAmounts.search(DOBPrograms.NJEDAPPE) + DOBPrograms.NJEDAPPE.length
        ...
        messageGenerator: app =>
            `Applicant not listed on Application, but in DOB List (NJEDA PPE1 or PPE2).`,
        severity: Decision.Review,
    ...
    },
    {
        ...
        messageGenerator: app =>
            `Applicant reported PPE Phase 2 funding approved or in progress but did not indicate the amount`,
        severity: Decision.Review,
        slug: 'MissingDOBAmount',
    },

    {
        ...
        messageGenerator: app =>
            `Applicant reported Sustain and Serve funding approved or in progress but did not indicate the amount`,
        severity: Decision.Review,
        slug: 'MissingDOBAmount',
    },
    {
        ...
        trigger: app => app.DOBValidation.DOBRecords.length > 0
            && app.DOBValidation.DOBFindings.includes(DOBPrograms.NJEDASSNJ)
            && (app.njeda_njedasustainandserve !== "Yes"
            || (app.DOBValidation.DOBAmounts.search(DOBPrograms.NJEDASSNJ) > 0 ? (Math.round(parseInt(app.DOBValidation.DOBAmounts.substr(app.DOBValidation.DOBAmounts.search(DOBPrograms.NJEDASSNJ) + DOBPrograms.NJEDASSNJ.length
        ...
        messageGenerator: app =>
            `Applicant either not listed on Application or less amount mentioned, but in DOB List (NJEDA SSNJ).`,
        severity: Decision.Review,
    ...
    ////////////////////// Legal Reviews ////////////////////////
    {
        ...
        messageGenerator: app =>
            `Additional information provided on background question #1 (convictions)`,
    ...
    },
    {
        ...
        messageGenerator: app =>
        `Additional information provided on background question #2 (denied licensure)`,
        severity: Decision.LegalReview,
        slug: null,
    },
    {
        ...
        messageGenerator: app =>
        `Additional information provided on background question #3 (public contractor subcontract ineligibility)`,
        severity: Decision.LegalReview,
        slug: null,
    },
    {
        ...
        messageGenerator: app =>
        `Additional information provided on background question #4 (violated the terms of a public agreement)`,
        severity: Decision.LegalReview,
        slug: null,
    },
    {
        ...
        messageGenerator: app =>
        `Additional information provided on background question #5 (injunction, order or lien)`,
        severity: Decision.LegalReview,
        slug: null,
    },
    {
        ...
        messageGenerator: app =>
        `Additional information provided on background question #6 (presently indicted)`,
        severity: Decision.LegalReview,
        slug: null,
    },
];
