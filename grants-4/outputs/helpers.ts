...
export function getQuarterlyWageHighestFTE(app: types.DecoratedApplication): types.QuarterlyWageData {
    if (app.wr30.notFound) {
        return {
            fteCount: null,
            quarterDesc: null,
    ...
    //var compare = stringSimilarity.compareTwoStrings(app.ContactInformation_BusinessName.trim().toUpperCase(), app.taxation["TAXREG Name"].trim().toUpperCase())
    //console.log("appname=" + app.ContactInformation_BusinessName.trim().toUpperCase());
    //console.log("taxname=" + app.taxation["TAXREG Name"].trim().toUpperCase());
    //console.log("compare=" + compare);
    interface WR30Record {
    ...
    const WEEKS_PER_QUARTER = 13.0;
    const FTE_QUARTERLY_MIN_WAGE = DOLLARS_PER_HOUR * HOURS_PER_WEEK * WEEKS_PER_QUARTER;
    var fteCount: number = 0;
    var quarterDesc: string = ``;

    //const wageRecords: WageRecord[] = <WageRecord[]>WR30_MAP.get(ein);
    const wr30Records: WR30Record[] =
        [{
            Year: 2010,
            Quarter: 1,
            FTE: 0
    ...
    app.wr30.wageRecords.forEach(function (value) {
        ...
        const year: number = value.Year; // app.wr30.wageRecords.map(record => record.Year);
        const quarter: number = value.Quarter; //app.wr30.wageRecords.map(record => record.Quarter);
    ...
    var yearfinal = maxXObject?.Year;
    var quaterfinal = maxXObject?.Quarter;
...
export function getWr30ReportingComments(app: types.DecoratedApplication): string {
    ...
    if (app.wr30.notFound) {
      comments.push(
        'Applicant did not file a WR-30.'
      );
    }

    // for changing bucket
    comments.push(BusinessTypeChanged(app));
...
export function getOwnershipStructure(app: types.DecoratedApplication): types.OwnershipStructures {
    ...
    const map = new Map<EntityType, types.OwnershipStructures>([
        ...
        [EntityType.Nonprofit_Organization, types.OwnershipStructures.Nonprofit],
        [EntityType.General_Partnership, types.OwnershipStructures.General_Partnership],
        [EntityType.Limited_Partnership, types.OwnershipStructures.Limited_Partnership],
        [EntityType.Government_Body, types.OwnershipStructures.Government_Body],
    ]);
...
export function getDobApproval(
    indicated: Application_YesNo,
    status: string   //DOB_Status | undefined
): types.ProgramApprovals | null {
    // program not selected (but value might be present from user changing their mind)
    if (!bool(indicated)) {
        return null;
...
export function isNotClearToTaxation(app: types.DecoratedApplication): boolean {
    return app.taxation['Clean Ind'] === TaxationCleanStatus.Not_Clear; // && hasNoTaxFilings(app);
    //return false;
}
...
export function getBusinessType(app: types.DecoratedApplication): string {
    ...
    const newBusinessType = BusinessTypeChanged(app);
    ...
    switch (businessType.toString()) {
        ...
        default:
            throw new Error(
                `Unexpected business type for application ${app.ApplicationId}`
            );
    }
