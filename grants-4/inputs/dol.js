...
function getEins(filePath) {
    ...
    return xlsx_1.default.utils.sheet_to_json(sheet, { defval: undefined, header: 1 })
        .flat()
        .filter(ein => ein && ein.toString().trim())
...
function addDolData(application) {
    const dol = {
        isActiveEmployer: ACTIVE_EMPLOYER_EIN_SET.has(application.Business_TIN.toString().trim()),
        uidNoGo: UID_NO_GO_EIN_SET.has(application.Business_TIN.toString().trim()),
        whdNoGo: WHD_NO_GO_EIN_SET.has(application.Business_TIN.toString().trim()),
    };
