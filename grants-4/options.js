...
function printStartMessage(options) {
    console.log(`Generating OLA Datas creation JSON for ${chalk.blue(options.count ? options.count : 'all')}${options.language ? ` ${chalk.blue(options.language)}` : ''} applications, skipping ${chalk.blue(options.skip || 0)}, ${options.county ? `filtering to only include ${chalk.blue(`${options.county} County`)}, ` : ''}from ${chalk.blue(options.base)}\n`);
}
