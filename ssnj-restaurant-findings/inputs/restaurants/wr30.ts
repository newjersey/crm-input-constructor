...
export function addWR30Data<T extends Restaurant>(restaurant: T): T & WR30 {
  ...
  const wr30: WR30Data = {
    notFound: WR30_NOT_FOUND_EINS.has(ein),
    wageRecords: WR30_MAP.get(ein) || [],
  };

  if (wr30.notFound && wr30.wageRecords.length) {
    throw new Error(`Restaurant ${restaurant.User} is on the WR-30 not-found list but does have ${wr30.wageRecords.length} wage records.`);
  }

  if (!wr30.notFound && !wr30.wageRecords.length) {
    throw new Error(`Restaurant ${restaurant.User} does't have any wage records but isn't on the WR-30 not-found list.`);
  }
